...
const EditCourseModal = ({ isOpen, onClose, onCourseUpdated, course }) => {
  ...
  useEffect(() => {
    if (course) {
      console.log("Loading course data for edit:", course);
      setFormData({
        title: course.title || "",
      ...
      setPreviewImage(course.thumbnail?.url || null);

      // Calculate discount percent
      if (course.discountedPrice && course.price) {
  ...
  const handleTopicChange = (chapterIndex, topicIndex, field, value) => {
    const updatedChapters = [...formData.chapters];
    if (!updatedChapters[chapterIndex].topics[topicIndex]) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (newThumbnail) {
        ...
        const cloudinaryResult = await uploadToCloudinary(newThumbnail);
        thumbnailData = {
      ...
      const courseData = {
        ...
        instructor: formData.instructor.trim(), // Make sure this is included
        ...
        chapters: formData.chapters.map((chapter) => ({
          ...
          topics: (chapter.topics || []).map((topic) => ({
            ...
            youtubeLink: topic.youtubeLink?.trim() || "",
      ...
      const response = await axios.put(`/courses/${course._id}`, courseData);
